...
namespace polaris
{
	...
	using HistoryTable = std::array<std::array<i32, 64>, 64>;
	...
	{
	public:
		MoveGenerator(const Position &pos, MovegenData &data, Move hashMove, HistoryTable *history = nullptr)
			: m_pos{pos},
			  m_moves{data.moves},
			  m_hashMove{hashMove},
	...
	private:
		...
		inline void scoreQuiet()
		{
			for (i32 i = m_noisyEnd; i < m_moves.size(); ++i)
			{
				...
				if (m_history)
					move.score = (*m_history)[static_cast<i32>(move.move.src())][static_cast<i32>(move.move.dst())];
